...
public class TypeChecker {
    ...
    public static Type Check(Function f, TypeEnv te) { //lab05
	    te.push(f.id, new ProtoType(f.type, f.params));
        for (Decl d : f.params) 
            te.push (d.id, d.type);

	    Type t = Check(f.stmt, te); // type check the function body (함수 본체 타입 검사)
	    if (t != f.type)
    ...
    static Type Check(Assignment a, TypeEnv te) { //공지 코드
    	if (a.ar != null) { // lab04 추가
