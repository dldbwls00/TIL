...
public class Sint {
    ...
    State Eval(Call c, State state) { //lab05 추가: call without return value
    	// evaluate call without return value
	
	    return state;
    }
 
    Value V (Call c, State state) { //lab05 추가: value-returning call
    ...
    State Eval(Return r, State state) { //lab05 추가: 함수 반환
        Value v = V(r.expr, state);
		return state.set(new Identifier("return"), v); 
    }

    State newFrame (State state, Call c, Function f) { //lab05 추가
    	//프레임 구성, 매개변수 전달
        if (c.args.size() == 0) 
            return state;
		
        //인자값 계산 -> 그 값을 val[]에 저장
        Value val[] = new Value[f.params.size()];
        int i = 0;
        for (Expr e: c.args)
        	val[i++] = V(e, state);
        
		//형식 매개변수들을 위한 기억공간 할당 (allocate 사용)
        state.push(new Identifier("return"), null);
        
        //인자값들을 형식 매개변수에 복사
        
        
        //프레임에 반환값 엔트리를 매개변수 바로 위에 추가
...
//      return null;
    ...
    State deleteFrame (State state, Call c, Function f) { //lab05 추가
	    //프레임에 반환값 엔트리 제거
...
//    	return null;
    ...
    State Eval(Empty s, State state) { return state; }
  
    State Eval(Assignment a, State state) { 
    	//lab04 수정: id[<expr>] = <expr>; //done
...
//		}
        ...
        for (int i=0; i<state.size(); i++){
            Pair p = state.get(i);
            if (a.ar.id.equals(p.id)){
                Value va = V(a.ar.expr, state);
                arr[va.intValue()] = v;
            }
        }
...
//                return state;
    ...
    State allocate (Decls ds, State state) {//lab03, lab04 수정
...
//				v = (Value) ds.get(i).expr;
				...
				//lab04 ektl
				if (ds.get(i).arraysize != 0){ //array
                    arr = new Value[ds.get(i).arraysize]; // 배열 선언에 대해서 배열
                    state.push(id, new Value(arr)); // (id, new Value(배열))을 state에 push
                } 
				else { //not array
                    v = (Value) ds.get(i).expr;
...
//            return state.get(v);
        ...
        if (e instanceof Array) { //lab04 done?
    		// id[<expr>]
    		Array ar = (Array) e; //배열 이름이 나타내는 배열에서
            Value va = V(ar.expr, state); 
            return (Value) (arr[va.intValue()]); //배열 요소 id[<expr>]의 값
    	}
